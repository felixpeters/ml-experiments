...
def show_dtypes(df):
	...
	types = c.most_common()
	for t in types:
    	print(f'{t[0]}: {t[1]} features') 
	return

def show_feats_with_missing_vals(df, missing_percs, axis=0):
    missing_vals = df.isnull().sum(axis=axis).sort_values(ascending=False) / df.shape[axis]
    for p in missing_percs:
        perc = len(missing_vals.loc[missing_vals > p]) / len(missing_vals)
        print('Features with more than {:.0f}% missing values: {:.1f}%'.format(p * 100, perc * 100))
	return
...
def show_cov_top_n_cats(df, n=10, dep_var=None):
    ...
    for cat in cat_vars:
        ...
        print(f'Coverage of top {n} categories for feature {col.name}: {top_n_count/total_count*100:.2f}%')
    return
